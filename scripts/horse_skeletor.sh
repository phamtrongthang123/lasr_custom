...
## my mod 
dataname=${seqname}
CUDA_VISIBLE_DEVICES=$dev python -m torch.distributed.launch --master_port $address --nproc_per_node=$ngpu optimize.py --name=$logname-6 --checkpoint_dir $checkpoint_dir --nosymmetric --nouse_gtpose --subdivide 3 --n_bones 45 --n_faces 2880 --n_hypo 1 --num_epochs $nepoch --model_path $checkpoint_dir/$logname-4/pred_net_latest.pth --dataname $dataname  --sil_path $sil_path  --ngpu $ngpu --batch_size $batchsize --opt_tex yes --sigval 1e-5
